...
// Offline render harness for Flux (mirrors caverns-delay's CavernsRenderIR / gradient-pitch's
// GradientRenderIR pattern - an audio-in-driven effect, not a synth). Reuses
// Source/Tests/TestCreateEditorStub.cpp (already exists for FluxTests) so this target stays free
// of the editor/LookAndFeel/BinaryData, matching every sibling RenderIR tool.
//
// Usage:
//   FluxRenderIR --out <path.wav> [--seconds 6] [--sampleRate 44100]
...
// --preset applies one of the 6 factory presets first (setCurrentProgram's own
...
// change at a block boundary partway through the render - proves the Grit/Brightness filter
// coefficient caches (see PluginProcessor.h's lastGritAmount/lastBrightnessAmount) invalidate
...
// cached "unchanged amount" value surviving a sample-rate change and producing wrong-rate
...
namespace
{
    ...
    void setParam(FluxAudioProcessor& processor, const std::string& paramID, float rawValue)
    {
    ...
    constexpr const char* allParamIDs[] = {
        FluxAudioProcessor::bypassParamID, FluxAudioProcessor::rateParamID,
        FluxAudioProcessor::syncParamID, FluxAudioProcessor::divisionParamID,
        FluxAudioProcessor::depthParamID, FluxAudioProcessor::shapeParamID,
        FluxAudioProcessor::stagesParamID, FluxAudioProcessor::offsetParamID,
        FluxAudioProcessor::feedbackParamID, FluxAudioProcessor::brightnessParamID,
        FluxAudioProcessor::gritParamID, FluxAudioProcessor::blendParamID,
    };

    // Same four-segment deterministic test signal design as caverns-delay/gradient-pitch's
    // RenderIR.cpp: a two-tone sine mix (harmonic content), a linear sweep, white noise, and a
    // silence tail - proportional to the total duration. A phaser's notch character needs
    // sustained tonal+noise content to show up meaningfully, not just an impulse.
    ...
    void renderPhase(FluxAudioProcessor& processor, const juce::AudioBuffer<float>& inputSignal,
...
int main(int argc, char* argv[])
{
    ...
    {
        std::fprintf(stderr,
            "Usage: FluxRenderIR --out <path.wav> [--seconds 6] [--sampleRate 44100]\n"
