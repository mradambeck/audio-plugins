...
// Drives the real IntruderAudioProcessor - specifically isBusesLayoutSupported()/processBlock()'s
// bus handling, which IntruderFDNEngineTests/IntruderParameterMapTests (the engine-only suite this
// target otherwise runs) can't reach at all, since that logic lives entirely in
// PluginProcessor.cpp. TestCreateEditorStub.cpp keeps this out of the GUI/LookAndFeel/font code,
// same as every other plugin's *ProcessorTests target in this repo.
namespace
{
...
class IntruderProcessorTests : public juce::UnitTest
{
public:
    IntruderProcessorTests() : juce::UnitTest("IntruderAudioProcessor", "Intruder") {}
    ...
    void runTest() override
    {
        ...
        beginTest("A mono input bus renders identically to a stereo input with the same signal on both channels");
        {
            ...
            IntruderAudioProcessor monoProcessor;
            juce::AudioProcessor::BusesLayout monoInStereoOut;
            monoInStereoOut.inputBuses.add(juce::AudioChannelSet::mono());
            monoInStereoOut.outputBuses.add(juce::AudioChannelSet::stereo());
            ...
            IntruderAudioProcessor stereoProcessor;
...
static IntruderProcessorTests intruderProcessorTests;
