...
class StrikeVoiceTests : public juce::UnitTest
{
public:
    using Voice = StrikeVoice<StrikeExcitation, LinearInterpolator>;

    StrikeVoiceTests() : juce::UnitTest("StrikeVoice", "Strike") {}
    ...
    void runTest() override
    {
        ...
        beginTest("Sustained loudness increases smoothly with bowAmount - no dead zone in the middle of the range");
        {
            ...
            for (float bow : bowSteps)
            {
                ...
                // leveler's own slow envelope (up to 0.6s time constant - see StrikeVoice.h) to
        ...
        beginTest("bowAmount = 0 still decays to silence on its own, like a plucked note");
        {
            ...
            // user-approved character change - see plugins/strike-synth/README.md) - so this no longer
        ...
        beginTest("Abruptly changing bowAmount mid-note keeps output continuous, no discontinuity spike");
        {
            ...
            // should change. The one-pole recurrence in StrikeExcitation is continuous by
        ...
        beginTest("Waveshape = 100% stays finite and bounded at the worst-case combination (max Decay, full Bow)");
        {
            ...
            // StrikeWaveFolder's own unconditional +-1 output bound survives being embedded in
            // the actual loop, not just in isolation (see StrikeWaveshaperTests.cpp).
            Voice voice;
            voice.prepare(44100.0);
        ...
        beginTest("Waveshape loudness parity: bowed (genuinely-folding) conditions don't get crushed or run away");
        {
            ...
            // StrikeWaveFolder's loudness fix (full `/drive` compensation applied to BOTH the
                                                 ...
                                                 std::make_tuple(1.0f, 1.0f, "bow max-decay (worst case)") })
            {
                ...
                // bow-noise term (see StrikeExcitation.h's own comment): the UNSHAPED ("off")
                ...
                // listen pass judged the noise too hiss-like - see StrikeExcitation.h) - 20 keeps
        ...
        // StrikeVoice.h's setDistortionPosition() comment for the exact scope.
        ...
        beginTest("A held bow note's loudness stays within a bounded range window-to-window (loudness leveling)");
        {
            ...
            // output leveler in StrikeVoice::renderNextSample(), down to ~1.2dB
            ...
            for (int note : { Voice::kLowestSupportedMidiNote, 48, 60, 72 })
            {
                ...
                for (int step = 1; step <= 20; ++step)
                {
                    ...
                    if (structureAmount <= 0.75f)
                    {
                        ...
                        // StrikeDispersionFilter's own comment for why this pushes closer to the
        ...
        beginTest("StrikeDispersionFilter is a true allpass (preserves magnitude) across gains and frequencies");
        {
        ...
        beginTest("Topology=Dual at Cross-Couple=0%, Detune=0%: bounded, same fundamental as Single, but NOT bit-identical to it");
        {
            ...
            // silently identical to Single, defeating the whole feature. See StrikeVoice.h's own
        ...
        {
            // The direct empirical validation of StrikeVoice.h's own closed-form safety
        ...
        {
            // The direct empirical validation of StrikeVoice.h's own closed-form safety argument
        ...
        // StrikeLoopFilter.h's own comment) - Resonance is just the filter's own Q now, and the
        ...
        beginTest("Loop Filter Type=Resonant at max Resonance produces bounded, decaying output across the full note range");
        {
            for (int note : { Voice::kLowestSupportedMidiNote, 60, Voice::kHighestSupportedMidiNote })
            {
                ...
                // all (see StrikeLoopFilter.h's own comment) - it would no longer be testing
        ...
        // - see StrikeLoopFilter.h/StrikeRingModulator.h's own comments), so decay time at a
...
static StrikeVoiceTests strikeVoiceTests;

class StrikeShortDelayTests : public juce::UnitTest
{
public:
    StrikeShortDelayTests() : juce::UnitTest("StrikeShortDelay", "Strike") {}

    void runTest() override
    {
        beginTest("delaySamples=0 returns the input immediately, bit-exact");
        {
            StrikeShortDelay delay;
            delay.prepare(100);
...
static StrikeShortDelayTests strikeShortDelayTests;
