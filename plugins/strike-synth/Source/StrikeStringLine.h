...
// the delay length that actually sets the note's pitch - see StrikeVoice.h.
...
class StrikeStringLine
{
public:
    // Allocates - only ever call this from StrikeStringLineChannel::prepare(), never from the
    // audio thread. maxDelaySamples should come from
    // StrikeStringLineChannel::requiredCapacitySamples() - never resized after this call.
    ...
    // portion of the delay ahead of a dispersion/allpass stage (Structure) - see StrikeVoice.h.
