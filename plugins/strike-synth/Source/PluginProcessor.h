...
// (StrikeExcitation.h, StrikeLoopFilter.h, StrikeStringLine.h, StrikeVoice.h) - see those
// files for the actual algorithm and the swap seams. Voice-to-note allocation/stealing is its own
// standalone, testable class (StrikeVoiceAllocator.h) - see that file for why it's separate.
class StrikeAudioProcessor : public juce::AudioProcessor
{
public:
    StrikeAudioProcessor();
    ~StrikeAudioProcessor() override;
...
private:
    ...
    // 8 voices, basic oldest-voice-stealing (see StrikeVoiceAllocator.h) - each Voice composes
    ...
    // StrikeExcitation::setBowForce()'s own comment.
    ...
    // StrikeVoice.h's own safety argument for why the full range is sweepable with no risk.
    // Detune is latched at noteOn instead (no smoothing needed), same convention as Brightness -
    // see StrikeVoice::noteOn()'s own comment for why real unison detuning isn't a live gesture.
    juce::SmoothedValue<float> crossCoupleSmoothed;

    // Couple Delay is also live/every-sample, same convention as Cross-Couple (see
    // StrikeVoice.h's own safety argument for why this needs no ceiling either).
    ...
    // convention as Waveshape/Structure - see StrikeLoopFilter.h's own comment for why none of
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(StrikeAudioProcessor)
};
