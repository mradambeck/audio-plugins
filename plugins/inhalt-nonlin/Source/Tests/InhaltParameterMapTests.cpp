...
public:
    ...
    void runTest() override
    {
        ...
        beginTest("mapHighKnobToTilt reproduces the two measured endpoints and interpolates monotonically");
        {
            ...
            expect(std::abs(atNine.lowGain - 1.62181f) < 1.0e-3f, "H=-9 low gain should match the measured +4.2dB");
            expect(std::abs(atNine.highGain - 0.350752f) < 1.0e-3f, "H=-9 high gain should match the measured -9.1dB");
        ...
        beginTest("mapTimeKnobToGateParams produces sane, finite values across the whole Time range");
        {
            for (float t = 0.1f; t <= 9.8f; t += 0.5f)
            {
                const auto gate = InhaltParameterMap::mapTimeKnobToGateParams(t);
