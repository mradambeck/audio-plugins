...
// carrying those artifacts. Deliberately does NOT run through any filter model (Architecture #3 -
// filters are playback-side only on the real hardware; the optional "double smear" that
// deliberately crosses that line arrives in Phase 5). Quantization always runs (using the current
// Bit Depth/Quantizer Mode - Phase 3) even when the resample+drive technique itself is bypassed,
// since on real hardware SOME converter is always in the signal path - `settings.bypass`
// specifically disables resample+drive+quantize ALL THREE as one unit ("monitor the raw source
// with nothing engaged"), not quantization alone.
class ConcreteCapturePass
{
public:
    struct Settings
    {
        bool bypass = true;                 // Disables resample+drive+quantize entirely - see the class comment.
