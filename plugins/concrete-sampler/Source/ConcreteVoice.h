...
// three machine modes - see ConcretePitchEngine.h) plus a basic ADSR amp envelope. The pitch
// engine, base rate, and coarse/fine tune are all snapshotted once at startNote() and held fixed
// for the voice's lifetime, the same way Phase 1 already snapshots the zone/velocity - matches
// this catalog's precedent (Strike's Mono/Topology switches) for "a mode selection is captured at
// note-on, not smoothly live-updated mid-note" rather than introducing a second, inconsistent
// live-parameter convention just for pitch.
//
...
public:
    ...
    // on why those are different things and which one applies when).
