...
void StrikeExcitation::prepare(double sampleRate) noexcept
{
    sampleRateHz = sampleRate;
    reset();
}

void StrikeExcitation::reset() noexcept
{
    lowpassState = 0.0f;
...
void StrikeExcitation::setBaseDuration(int delaySamples) noexcept
{
...
void StrikeExcitation::noteOff() noexcept
{
    if (stage != Stage::Idle)
        stage = Stage::Release;
}

float StrikeExcitation::nextPinkNoiseSample(float white) noexcept
{
...
float StrikeExcitation::nextBrownNoiseSample(float white) noexcept
{
    brownState += brownLowpassCoeff * (white - brownState);
    return brownState * brownNormalizationGain;
}

float StrikeExcitation::nextBowPinkNoiseSample(float white) noexcept
{
...
float StrikeExcitation::nextBowBrownNoiseSample(float white) noexcept
{
    bowBrownState += brownLowpassCoeff * (white - bowBrownState);
    return bowBrownState * brownNormalizationGain;
}

float StrikeExcitation::nextNoiseSample() noexcept
{
...
float StrikeExcitation::nextBowNoiseSample() noexcept
{
...
float StrikeExcitation::nextFrictionSample(float stringSignal) noexcept
{
...
float StrikeExcitation::nextExcitationSample(float velocity01, float stringSignal) noexcept
{
    if (stage == Stage::Idle)
        return 0.0f;
