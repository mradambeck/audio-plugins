...
namespace
{
    constexpr float strikeWaveshaperPi = 3.14159265358979323846f;
}
...
//
// Unlike Excitation/Loop Filter/Delay Tuning (each a compile-time StrikeStringLineChannel template
// parameter - pick one implementation, rebuild to change it), this seam is a RUNTIME choice: the
// user asked for a dropdown to swap character live, not a rebuild. StrikeStringLineChannel owns
...
// PluginProcessor/StrikeVoice.h's call sites don't need to know which one is active.
//
// StrikeFuzz (asymmetric tanh clipping) and StrikeSaturator (symmetric tanh soft saturation)
...
//
// StrikeWaveFolder: a classic triangle/sine wavefolder (the "West Coast synthesis" style
...
// asymmetric clipping, folding) for pushing Strike past a physically-plausible plucked string.
//
...
// implementation (StrikeWaveFolder::updateEnvelope(), called once per sample from
// StrikeStringLineChannel::renderChannelSample()) rather than rebuilding it from scratch.
class StrikeWaveFolder
{
public:
    ...
    //     stringLine.write() - see StrikeVoice.h), that extra gain compounds every pass -
    ...
    float process(float x, float amount01, float driveCompensation = 1.0f) const noexcept
    {
        ...
        const auto folded = threshold * (2.0f / strikeWaveshaperPi)
                           * std::asin(std::sin(driven * strikeWaveshaperPi / (2.0f * threshold)));
...
// StrikeBitCrush: sample-and-hold rate reduction + bit-depth quantization, combined under one
...
// seam (see this file's own top comment and `StrikeVoice.h`'s branch).
//
...
// risk to guard against with a `driveCompensation` split - see StrikeWaveFolder's own comment for
...
// same `amount01` rather than split into two separate knobs, since Strike currently only exposes
